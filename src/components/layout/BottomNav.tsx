        ...
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[11px] mt-0.5 tracking-tight truncate max-w-[64px] transition-colors">
                {tab.label}
              </span>
              {isActive && (
                <span className="absolute bottom-0.5 w-3.5 h-0.5 bg-slate-900 dark:bg-white rounded-full animate-scale-in" />
              )}
            </button>
          );
        })}
