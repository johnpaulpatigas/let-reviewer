...
export const StudyMaterialsPage: React.FC<StudyMaterialsPageProps> = ({
  ...
  return (
    <div className="space-y-4 animate-page-enter">
      ...
      <div className="flex items-center justify-between py-2 px-3 bg-slate-100 dark:bg-neutral-900 rounded-md text-xs text-slate-600 dark:text-neutral-400 border border-slate-200 dark:border-neutral-800">
        <div>
        ...
        <div className="w-24 sm:w-32 bg-slate-200 dark:bg-neutral-800 rounded-full h-1.5 overflow-hidden">
          <div
      ...
      {/* Category Pills */}
      <div className="flex gap-1 p-1 bg-slate-100 dark:bg-neutral-900 rounded-md overflow-x-auto">
        <button
          type="button"
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded border text-[11px] font-medium whitespace-nowrap transition-colors cursor-pointer ${
              selectedSubjectId === 'all'
                ? 'bg-slate-900 text-white dark:bg-white dark:text-neutral-900 border-transparent font-bold'
        ...
        {paginatedMaterials.map((material) => {
          ...
          return (
            <div
              ...
              className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 hover:border-slate-400 dark:hover:border-neutral-600 rounded-lg p-4 transition-all duration-150 active:scale-[0.99] cursor-pointer group flex flex-col justify-between space-y-3"
            >
              <div>
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  <div className="flex flex-wrap items-center gap-1.5 min-w-0">
                    <CategoryBadge category={material.category} size="sm" />
                    <span className="text-xs font-medium text-slate-500 dark:text-neutral-400 truncate max-w-[150px] sm:max-w-[240px]">
                ...
                <h3 className="font-bold text-slate-900 dark:text-white text-sm sm:text-base leading-snug group-hover:text-slate-700 dark:group-hover:text-neutral-200 transition-colors">
                  {material.title}
                </h3>

                <p className="text-xs text-slate-600 dark:text-neutral-400 line-clamp-2 mt-1 leading-relaxed">
                  {material.description}
                </p>
              </div>

              <div className="pt-2 border-t border-slate-100 dark:border-neutral-800 flex items-center justify-between text-xs">
                <div className="flex items-center gap-2.5 text-slate-500 dark:text-neutral-400 text-[11px]">
                ...
                <div className="flex items-center gap-1 text-slate-400 dark:text-neutral-400 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">
      ...
      {filteredMaterials.length === 0 && (
        <div className="text-center py-10 bg-white dark:bg-neutral-900 rounded-lg border border-slate-200 dark:border-neutral-800 p-5 space-y-2">
          <BookOpen className="w-8 h-8 text-slate-400 dark:text-neutral-500 mx-auto" />
          <h3 className="font-bold text-slate-800 dark:text-neutral-200 text-sm">
